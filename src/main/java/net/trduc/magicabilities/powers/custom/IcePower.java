...
import net.trduc.magicabilitiesfork.misc.GeneralMethods;
...
public class IcePower extends Power implements IdlePower {
    ...
    private static final String ice_freeze_counter = "ice.freeze-counter";
    ...
    private void explode(LeftClickExecute ex, double initM) {
        ...
        particleApi.spawnParticles(p.getLocation(), Particle.FIREWORK, 30, 0.01, 0.01, 0.01, 0.5);
        particleApi.spawnParticles(p.getLocation(), Particle.SNOWFLAKE, 30, 0.01, 0.01, 0.01, 0.3);
    ...
    private void moveExecution(MoveExecute ex){
        ...
        freezeLiquidUnderFeet(event.getTo());
    }
    private void freezeLiquidUnderFeet(Location loc) {
        Block sb = loc.clone().add(0, -1, 0).getBlock();
        if (!sb.isLiquid()) return;
        switch (sb.getType()) {
            case WATER:
    ...
    private void damagedByExecute(DamagedByExecute ex){
        Player p = ex.getPlayer();
        if (CooldownApi.isOnCooldown(ice_freeze_counter, p)) return;

        Entity damager = ((EntityDamageByEntityEvent) ex.getRawEvent()).getDamager();
        if (!(damager instanceof LivingEntity)) return;

        addCd(ice_freeze_counter, p);
        damager.setFreezeTicks(damager.getMaxFreezeTicks()*4);
    }
        ...
        new BukkitRunnable(){
            ...
            public void run() {
                ...
                if (!as.getLocation().clone().getBlock().isPassable() || l){
                    if (l) freezeLiquidUnderFeet(as.getLocation().clone().add(0, 1, 0));
                    if (!as.isDead()){
                        spawnSpellHitParticles(as.getLocation().clone());
                        as.remove();
                        cancel();
    ...
    private void spawnSpellHitParticles(Location l){
        ...
        particleApi.spawnParticles(l, Particle.SNOWFLAKE, 30, 1, 1, 1, 1);
